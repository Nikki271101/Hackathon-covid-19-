...
#please first install package from pip install win32com.client
...
class change_country_state(object):
    ...
    def finding_avg_no_of_days_for_country_to_change(self):
        df=pd.read_csv(open('G:\\phytho\\uma mam\\hackathon\covid_19_deaths.csv','r'))
        i=4
        i1=0
...
def country_took_the_maximum_number_of_days_to_death(state1):
        df=pd.read_csv(open(f"G:\\phytho\\uma mam\\hackathon\covid_19_{state1}.csv",'r'))
...
while(c==1):
    ...
    p=0
    while(p==0):
        st=int(input("enter the state you need to find out average days taken to a country to turn in\n1.death\n2.recovered\n"))
    # print(country_name)
        if (st==1):
            ...
            df=pd.read_csv(open('G:\\phytho\\uma mam\\hackathon\covid_19_recovered.csv','r'))
            recover=change_country_state(country_name,states[1])
            recover.finding_avg_no_of_days_for_country_to_change()
        speak("press 0 to again loop into same country else press any key to terminate the program")
        p=int(input("press 0 to again loop into same country else press any key to terminate the program"))
    
    speak("press 1 to loop into other country else press any key to terminate the program")
    c=int(input("press 1 to again loop into other country else press any key to terminate the program"))
